...
class FiltersTab(AnalysisTab):
    def __init__(self, main):
        ...
        # ---- sRNA genomic location filter (from tab 01 output) ----
        lbox = QGroupBox("sRNA location filter (optional - uses the "
                         "01 Position Classification output)")
        lf = QFormLayout(lbox)
        self.loc_enabled = self.QCheckBox("Enable location filter")
        self.loc_enabled.setChecked(False)
        self.loc_enabled.setToolTip(
            "When ticked, only sRNA-target pairs whose sRNA falls in one of the "
            "selected genomic categories are kept. The annotation file is the "
            "table produced by the 01 Position Classification tab."
        )
        lf.addRow(self.loc_enabled)
        self.loc_file = QLineEdit()
        self.loc_file.setPlaceholderText("sRNA annotation file from tab 01 (.xlsx/.csv)")
        self.loc_file.setToolTip(
            "The sRNA_annotation table from the 01 Position Classification tab. "
            "Must contain the sRNA ID column and a 'location' column."
        )
        lf.addRow("Annotation file:", browse_file_row(self.loc_file, self,
    ...
    def run(self):
        ...
        params = {
            ...
            "location_filter_enabled": self.loc_enabled.isChecked(),
