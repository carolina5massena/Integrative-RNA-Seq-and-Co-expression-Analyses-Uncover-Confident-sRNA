...
class FiltersTab(AnalysisTab):
    def __init__(self, main):
        ...
        self.e_inta = self._dspin(-2.44, -1e6, 1e6, 2); self.e_inta_on = QCheckBox(); self.e_inta_on.setChecked(False)
        ef.addRow("E_intaRNA max (<=):", self._with_toggle(self.e_inta, self.e_inta_on))
        self.e_rnaplex = self._dspin(-32.6, -1e6, 1e6, 2); self.e_rnaplex_on = QCheckBox(); self.e_rnaplex_on.setChecked(False)
        ef.addRow("E_Rnaplex max (<=):", self._with_toggle(self.e_rnaplex, self.e_rnaplex_on))
        self.e_target = self._dspin(-5.13, -1e6, 1e6, 2); self.e_target_on = QCheckBox(); self.e_target_on.setChecked(False)
        ef.addRow("E_TargetRNA3 max (<=):", self._with_toggle(self.e_target, self.e_target_on))
        self.p_target = self._dspin(0.06, 0, 1, 3); self.p_target_on = QCheckBox(); self.p_target_on.setChecked(False)
        ef.addRow("Probability_TargetRNA3 min (>=):", self._with_toggle(self.p_target, self.p_target_on))
        self.p_rf = self._dspin(0.45, 0, 1, 3); self.p_rf_on = QCheckBox(); self.p_rf_on.setChecked(True)
        ...
        self.deg_basemean_on = QCheckBox(); self.deg_basemean_on.setChecked(True)
        ...
        self.deg_min_basemean.setValue(10.0)
        ...
        self.lfc_up_on = QCheckBox(); self.lfc_up_on.setChecked(False)
        ...
        self.lfc_down_on = QCheckBox(); self.lfc_down_on.setChecked(False)
        ...
        self.consensus_mode.setCurrentIndex(1)  # default: presence (ignore log2FC)
        ...
        self.min_strains = QSpinBox(); self.min_strains.setRange(1, 1000); self.min_strains.setValue(5)
        ...
        self.selection_on = self.QCheckBox(); self.selection_on.setChecked(True)
