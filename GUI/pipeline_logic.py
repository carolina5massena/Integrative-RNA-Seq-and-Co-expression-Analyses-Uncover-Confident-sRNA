...
def run_filters(p, log):
    ...
    # ---- 2b. sRNA genomic location filter (optional) -----------------------
    # Uses the annotation table produced by the 01 Position Classification tab.
    # Each sRNA is mapped to its 'location' category and pairs are kept only if
    # the sRNA falls in one of the requested categories.
    if p.get("location_filter_enabled", False):
        ...
        log(f"After location filter ({', '.join(keep)}): {len(pred)} "
            f"(from {before}; sRNAs not found in the annotation file are dropped)")
    else:
        log("Location filter skipped.")
